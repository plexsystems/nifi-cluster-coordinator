...
def sync(cluster: Cluster, project: Project, project_cluster: ProjectCluster):
    ...
    for environment in desired_environments:
        if environment.name in current_environments_json_dict:
            _update(cluster, project, project_cluster, environment, current_environments_json_dict[environment.name])
        else:
            _create(cluster, project, project_cluster, environment)


def _create(cluster: Cluster, project: Project, project_cluster: ProjectCluster, environment: ProjectEnvironment):
    logger = logging.getLogger(__name__)
    ...
    try:
        ...
        if response.status_code != 201:
            ...
            return
        logger.info(f'Created project: {project.name}, environment: {environment.name}.')
        _update(cluster, project, project_cluster, environment, response.json())
    except requests.exceptions.RequestException as exception:
        logger.warning(f'Unable to create project: {project.name}, environment: {environment.name}.')
    ...
    if (
        ...
        if 'versionControlInformation' in environment_json['component']:
            try:
                ...
                logger.warning(f'Unable to temporarily delete version control from project: {project.name}, environment: {environment.name}.')
                logger.warning(exception)
                return
        ...
        try:
            ...
            if response.status_code != 200:
                logger.warning(response.text)
                return
            logger.info(f'Updated project: {project.name}, environment: {environment.name}.')
            environment_json = response.json()
        except requests.exceptions.RequestException as exception:
            logger.warning(f'Unable to update project: {project.name}, environment: {environment.name}.')
            logger.warning(exception)
...
def _delete(cluster: Cluster, project: Project, delete_environment_json):
    ...
    try:
        ...
        if response.status_code != 200:
            logger.warning(response.text)
            return
        logger.info(f'Deleted project: {project.name}, environment: {environment_name}')
    except requests.exceptions.RequestException as exception:
        logger.warning(f'Unable to delete project: {project.name}, environment: {environment_name}.')
        logger.warning(exception)
...
def _update_version(desired_version, cluster: Cluster, project: Project, environment: ProjectEnvironment, environment_json):
    ...
    try:
        ...
        if response.status_code != 200:
            logger.warning(response.text)
            return
        logger.info(f'Updated project: {project.name}, environment: {environment.name}, version: {desired_version}.')
    except requests.exceptions.RequestException as exception:
        logger.warning(f'Unable to update project: {project.name}, environment: {environment.name}, version: {desired_version}.')
        logger.warning(exception)
